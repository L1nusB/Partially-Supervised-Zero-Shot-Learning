...
class CoupledHeadModule(nn.Module):
    def __init__(self, body: Sequence[nn.Module], fc_final: nn.Linear, detach: bool = False) -> None:
        super().__init__()
        self.body = nn.Sequential(*body)
        self.final = fc_final
        self.detach = detach
        self.forward = self.forward_detached if detach else self.forward_attached
        
    def forward_attached(self, x: torch.Tensor, additional: Optional[torch.Tensor]=None) -> torch.Tensor:
        pre_final = self.body(x)
        if additional is None:
            final = self.final(pre_final)
        else:
            final = self.final(torch.cat((pre_final, additional), dim=1))
        return final
    
    def forward_detached(self, x: torch.Tensor, additional: Optional[torch.Tensor]=None) -> torch.Tensor:
        pre_final = self.body(x)
        if additional is None:
            final = self.final(pre_final)
        else:
            final = self.final(torch.cat((pre_final, additional.detach()), dim=1))
        return final
    
class CoupledHead(CustomHead):
    ...
    def __init__(self, 
                 ...
                 coupling: Literal['forward','backward'] = 'forward',
                 coupling_levels: Optional[int] = 1,
        ...
        """Construct a coupled hierarchical head that returns a prediction for each level of the hierarchy.
        Depending on `coupling` predictions of the previous or following level are used as additional input.
        The number of levels between which predictions are coupled can be specified via `coupling_levels`.
        If `None` the coupling is done between all levels.
        Sharing is performed by concatenating the final output onto the input of the final layer of the next level.
        If `detach_coupling` is set to `True` the coupling tensor is detached before concatenation.	
        ...
        Args:
            in_features (int): 
                Number of features in the input tensor comming from the backbone.
            out_features (int | Sequence[int]): 
                Number of output features for each hierarchy level.
            hierarchy_levels (Optional[int]): 
                Number of hierarchy levels. Defaults to None.
            head_depth (int | Sequence[int]): 
                Depth of each subhead i.e. each hierarchy level. Defaults to 1.
            heads (Optional[Sequence[nn.Module]]): 
                List of heads to use instead of constructing new ones.
            ...
            head_indices (Optional[Sequence[int]]): 
                Indices of the hierarchy level a given head should replace.
                If not specified the first `len(heads)` levels are replaced. Defaults to None.
            coupling (Literal['forward','backward']):
                Coupling direction of the predictions. `forward` will share features from coarse to fine. 
                `backward` will share features from fine to coarse. Defaults to 'forward'.
            coupling_levels (Optional[int]):
                Number of levels between which predictions are coupled. If `None` all levels are coupled. Defaults to None.
            detach_coupling (bool):
                If `True` the coupling tensor is detached before concatenation. Defaults to False.
            intermediate_features (Optional[Sequence[int] | Sequence[Sequence[int]]]): 
                Intermediate feature sizes if head_depth>1. Defaults to None.
            reduction_factor (float | Sequence[float] | Sequence[Sequence[float]]): 
                Reduction factor for automatic calculation of intermediate_features if head_depth>1. Defaults to 2.0.
        ...
        """
        super(CoupledHead, self).__init__(in_features=in_features,
                                               out_features=out_features,
                                               head_depth=head_depth,
                                               intermediate_features=intermediate_features,
                                               reduction_factor=reduction_factor)
        assert coupling in ['forward','backward'], f"Coupling must be one of ['forward','backward'], got {coupling}"
        self.coupling = coupling
        self.detach_coupling = detach_coupling
        self.forward = self.forward_forward if coupling == 'forward' else self.forward_backward
        ...
        if coupling_levels is None:
            coupling_levels = self.hierarchy_levels - 1
        self.coupling_levels = min(coupling_levels, self.hierarchy_levels-1)
        ...
        body_layers = [[nn.Linear(self.features[i][j], self.features[i][j+1]) for j in range(self.head_depth[i]-1)]
                                    for i in range(self.hierarchy_levels)]
        fc_finals = self._construct_final_layers()
        ...
        head_modules = [CoupledHeadModule(body, fc_finals[i], detach=self.detach_coupling) 
                        for i, body in enumerate(flattened_bodies)]
    ...
    def forward_forward(self, x:torch.Tensor) -> List[torch.Tensor]:
        y = [x.clone()] * self.hierarchy_levels
        # First level has no coupling as nothing to get additional input from
        y[0] = self.heads[0](y[0])
        for i in range(1, self.hierarchy_levels):
            # Get the coupling tensor from the previous level(s) based on coupling_levels
            y[i] = self.heads[i](y[i], torch.cat(y[max(0,i-self.coupling_levels):i], dim=1))
        return y
    
    def forward_backward(self, x:torch.Tensor) -> List[torch.Tensor]:
        y = [x.clone()] * self.hierarchy_levels
        y[-1] = self.heads[-1](y[-1])
        for i in range(self.hierarchy_levels-2, -1, -1):
             # Get the coupling tensor from the previous level(s) based on coupling_levels
             # No need for clipping as indexing will ignore values outside the list
            y[i] = self.heads[i](y[i], torch.cat(y[i+1:i+1+self.coupling_levels], dim=1))
        return y
    ...
    def head_kwargs(**kwargs) -> dict:
        ...
        head_params["hierarchy_levels"] = kwargs.get(CoupledHead.num_head_pred_key)
        head_params["coupling"] = kwargs.get("coupling", "forward")
        head_params["coupling_levels"] = kwargs.get("coupling_levels", 1)
        head_params["detach_coupling"] = kwargs.get("detach_coupling", False)
        return head_params
    
    def _construct_final_layers(self) -> List[nn.Linear]:
        if self.coupling == 'forward':
            # Forward sharing
            # First layer has nothing to get additional input from
            final_layers = [nn.Linear(self.features[0][-1], self.out_features[0])]
            for i in range(1, self.hierarchy_levels):
                num_additional = sum([self.out_features[j].item() for j in range(max(0,i-self.coupling_levels), i)])
                final_layers.append(nn.Linear(self.features[i][-1]+num_additional, self.out_features[i]))
            return final_layers
        else:
            # Backward sharing
            # Last layer has nothing to get additional input from
            final_layers = [nn.Linear(self.features[-1][-1], self.out_features[-1])]
            for i in range(self.hierarchy_levels-2, -1, -1):
                print(i+1, min(self.hierarchy_levels-1,i+1+self.coupling_levels))
                num_additional = sum([self.out_features[j].item() for j in range(i+1, min(self.hierarchy_levels,i+1+self.coupling_levels))])
                final_layers.insert(0, nn.Linear(self.features[i][-1]+num_additional, self.out_features[i]))
            return final_layers
    ...
    def _validate_and_set_attributes(self, 
        ...
        self.intermediate_features : Sequence[Optional[Sequence[int]]] = intermediate_features
