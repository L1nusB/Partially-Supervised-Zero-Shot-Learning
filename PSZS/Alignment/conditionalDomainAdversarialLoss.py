...
class ConditionalDomainAdversarialLoss(nn.Module):
    r"""Adapted from `~tllib.alignment.cdan.ConditionalDomainAdversarialLoss` to use
    BCEWithLogitsLoss instead of binary cross entropy loss to enable autocasting (AMP).
    ...
    def __init__(self, domain_discriminator: nn.Module, entropy_conditioning: Optional[bool] = False,
                 ...
                 reduction: Optional[str] = 'mean', binary_pred: bool=True, use_logits: bool = True):
    ...
    def forward(self, g_s: torch.Tensor, f_s: torch.Tensor, g_t: torch.Tensor, f_t: torch.Tensor) -> torch.Tensor:
        ...
        if self.binary_pred:
            ...
            self.domain_discriminator_accuracy = self.binary_accuracy_func(d, d_label)
            if self.entropy_conditioning:
                return self.binary_loss_func(d, d_label, weight.view_as(d), reduction=self.reduction)
            else:
                return self.binary_loss_func(d, d_label, reduction=self.reduction)
        else:
            d_label = torch.cat((
