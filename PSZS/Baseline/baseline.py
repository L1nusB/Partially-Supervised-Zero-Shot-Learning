...
def main(args):
    ...
    if ((bs_source & (bs_source-1) == 0) and bs_source != 0)==False:
        warnings.warn(f"Batch size for source domain {bs_source} is not a power of 2.")
    if ((bs_target & (bs_target-1) == 0) and bs_target != 0)==False:
        warnings.warn(f"Batch size for target shared domain {bs_target} is not a power of 2.")
