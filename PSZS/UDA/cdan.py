...
def main(args: argparse.Namespace):
    ...
    print(args)
    filewriter.save_args(args, logger.root)
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train_cls_accs, train_domain_accs, train_losses, train_trans_losses = train(train_source_iter, train_target_iter, classifier, domain_adv, optimizer,
        ...
        if acc1 > best_acc1 or osp.exists(logger.get_checkpoint_path('best'))==False:
            shutil.copy(logger.get_checkpoint_path('latest'), logger.get_checkpoint_path('best'))
        best_acc1 = max(acc1, best_acc1)
        metrics.update([('train_cls_accs', train_cls_accs),
                        ('train_domain_accs', train_domain_accs),
                        ('train_losses', train_losses), 
                        ('train_trans_losses', train_trans_losses),
                        ('eval_acc1', acc1),
                        ('best_acc1', best_acc1)])
        
        filewriter.update_summary(epoch=epoch+1, metrics=metrics, root=logger.root,
    ...
    metrics = OrderedDict()
    metrics.update([('test_acc1', acc1)])
    summary = filewriter.update_summary(epoch='Test', metrics=metrics, root=logger.root,
                                  write_header=True)
    filewriter.convert_csv_to_excel(summary)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: ImageClassifier,
    ...
    return cls_accs.avg, domain_accs.avg, losses.avg, trans_losses.avg
