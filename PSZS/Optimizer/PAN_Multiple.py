...
class PAN_Multiple(Base_Multiple):
    def __init__(self, train_iters: Sequence[ForeverDataIterator], 
                 ...
                 max_smoothing_epochs: int = 10,
                 **optim_kwargs,
                 ) -> None:
        self.entropy_loss_weights = entropy_loss_weights
        if self.compute_entropy == False:
    ...
    def _compute_loss_cls(self, 
        ...
        fine_loss_components = [F.kl_div(F.log_softmax(p[self.model.classifier.test_head_pred_idx], dim=1), l[0], reduction='batchmean') 
                     for p,l in zip(pred, smoothed_fine_labels)]
    ...
    def _compute_loss_features(self, features: Sequence[FEATURE_TYPE]) -> torch.Tensor:
        ...
        return entropy_loss + super()._compute_loss_features(features=features)
